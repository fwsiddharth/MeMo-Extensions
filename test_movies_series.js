...
async function testItem(name, typeHint) {
  console.log(`\n=========================================`);
  ...
  try {
    ...
    if (res && res.length > 0) {
      // Find best match if typeHint is provided, else use first
      if (typeHint) {
         tmdbAnime = res.find(r => r.format === typeHint) || res[0];
      } else {
         tmdbAnime = res[0];
      }
      console.log(`[Vidlink] Found Match: ${tmdbAnime.title} (${tmdbAnime.format} - ${tmdbAnime.seasonYear}) - ID: ${tmdbAnime.id}`);
      ...
      if (eps.episodes && eps.episodes.length > 0) {
        ...
        if (streamData) {
          ...
          console.log("[Vidlink] No episodes found.");
      }
    } else {
...
async function run() {
  await testItem("Alpha");
  await testItem("Hasee Toh Phasee");
  await testItem("From", "TV");
}
